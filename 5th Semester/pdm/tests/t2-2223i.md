...
# 7
```
No âmbito do modelo de programação disponibilizado pela biblioteca Room, pode-se afirmar que:

- as classes anotadas com @Entity representam os dados a armazenar

- as interfaces anotadas com @Dao caracterizam as operações de acesso a dados 
cuja implementação é fornecida durante a fase de build
...
Fazer startActivity a uma activity que já está started não faz nada.
```
...
Tendo em conta o histórico, fazer back vai destruir a activity, no entanto, o 
onStartCounter é incrementado porque a activity é recriada (devido a termos 
navegado da mesma activity para a mesma activity).
```
...
Quando se faz back, a activity é destruida e o viewmodel também.
```