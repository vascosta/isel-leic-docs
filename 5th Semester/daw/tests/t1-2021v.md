...
Acções representam operações que podem ser executadas sobre o recurso atual.
```
...
class HandlersService {
    val handlersMap = mutableMapOf<String, HandlerData>()
...
function App({ uri }: ComponentProps) {
