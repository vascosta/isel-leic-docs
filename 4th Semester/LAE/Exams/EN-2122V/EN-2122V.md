...
```
TR -> TR --> Casting -> mantem o valor da instância mas altera a referência

TP -> TP --> Coercion -> muda o valor da referência
...
val p = Pack(n) --> n(Int) -> v(Any) => TP -> TF => autoboxing
val res = p.v as Int? --> casting
```
...
Generacional:
* Maior performance;
* Normalemente, novos objetos têm um tempo de vida mais curto, logo o GC é mais eficiente nas gerações mais novas.
```
