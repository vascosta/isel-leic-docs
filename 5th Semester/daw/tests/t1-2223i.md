...
```
Só podem ser chamadas dentro de componentes React ou dentro de uma função hook customizada (função javascript cujo nome começa por use)
...
Estes beans por sua vez são guardados no contexto da aplicação e podem ser usados em qualquer parte da aplicação, uma vez que o Spring gere as dependências e o ciclo de vida dos beans.
```
...
Módulo -> mecânismo de organizar e isolar código, no entanto se a aplicação tiver muitos módulos, o browser vai ter de carregar muitos ficheiros, o que pode impactar a performance da aplicação

Ao usar o webpack, podemos criar um bundle com todos os módulos da aplicação (bundling), o que permite reduzir o número de pedidos HTTP ao servidor e melhorar a performance da aplicação.

Webpack permite também Lazy Loading, ou seja, carregar módulos apenas quando são necessários, o que permite reduzir o tamanho do bundle inicial e melhorar a performance da aplicação.
```
...
@Component
class HandlersInterceptor(private val handlersService: HandlersService) : HandlerInterceptor {
